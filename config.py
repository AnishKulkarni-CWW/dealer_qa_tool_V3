...
class QAConfig:
    ...
    # If the anchor is missed, a blind fixed fraction can cut the banner
    # copy off entirely — measured on a real BMW "THE 7" master creative,
    # whose banner text sits at almost exactly 35% of the image height. The
    # crop is therefore allowed to extend, one small step at a time, until
    # it actually contains text, but never past this ceiling (which stays
    # comfortably above the body copy on every real emailer creative).
    crop_anchor_fallback_max_ratio: float = 0.60
    crop_anchor_fallback_step_ratio: float = 0.05
    ...
    # ---- Banner Dealer Name: what to do when the banner has no dealer line ----
    # Many BMW email templates deliberately carry no dealer name on the
    # banner at all — the dealer block lives in the email body, which the
    # separate opt-in "Dealer exists in Banner" check already reports on.
    # Previously Banner Text QA fell back to comparing the expected dealer
    # name against the WHOLE banner text blob in that situation, which
    # produced a guaranteed Fail plus a meaningless "Unexpected/extra
    # word(s) nearby: ALL, IN, NO, MORE, EXCUSES, DRIVE, YOUR, ..." list on
    # every single adapt. It is now reported once, clearly, as a WARN.
    # Set this to True to treat a banner with no dealer line as a hard Fail
    # (appropriate only for templates where the dealer name really is
    # supposed to be rendered on the banner artwork).
    banner_dealer_name_missing_is_fail: bool = False
